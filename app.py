...
st.markdown("""
    <div style='text-align: center; font-size: 36px; font-weight: bold; color: black; margin-top: 20px;'>
...
# Image
st.image("ev-car-factory.jpg", use_column_width=True)  # Or any desired width


# Instruction line
st.markdown("""
    <div style='text-align: left; font-size: 22px; padding-top: 10px; color: black; font-weight: bold;'>
...
st.markdown('<span style="color:black ;">Select a County</span>', unsafe_allow_html=True)
...
st.markdown(
    f"<h3 style='color:black;'>📊 Cumulative EV Forecast for {county} County</h3>",
    unsafe_allow_html=True
)
...
if historical_total > 0:
    ...
    st.markdown(
    f"""<div style='background-color:#d4edda; padding:10px; border-left:5px solid green; border-radius:5px; color:black;'>
    Based on the graph, EV adoption in <b>{county}</b> is expected to show a <b>{trend} of {forecast_growth_pct:.2f}%</b> over the next 3 years.
    </div>""",
    unsafe_allow_html=True
)
...
# === New: Compare up to 5 counties ===
st.markdown("---")
st.markdown(
    "<h1 style='color:black; font-size: 30px;'>Compare EV Adoption Trends for up to 5 Counties</h1>",
    unsafe_allow_html=True
)


st.markdown('<span style="color:black;">Select up to 5 counties to compare</span>', unsafe_allow_html=True)
multi_counties = st.multiselect("Select up to 5 counties to compare",
    county_list,
    label_visibility="collapsed")
...
if multi_counties:
    ...
    # Plot
    st.markdown(
    "<h3 style='color:black;'>📈 Comparison of Cumulative EV Adoption Trends</h3>",
    unsafe_allow_html=True
)
    ...
    st.markdown(
    f"""<div style=' color:black; padding:10px; border-left:5px solid yellow; background-color: #fff9c4; border-radius:5px;'>
...
st.markdown(
    "<p style='color:black; font-size:18px; text-align:center;'><strong>Prepared for the AICTE Internship Cycle 2 by Purvi Porwal</strong></p>",
    unsafe_allow_html=True
)
